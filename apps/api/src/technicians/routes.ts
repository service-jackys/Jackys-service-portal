...
import { TechnicianServiceError, type TechnicianService } from './service.js';

function serviceError(error: unknown, response: Parameters<RequestHandler>[1]): void {
  if (!(error instanceof TechnicianServiceError)) throw error;
  problem(response, 404, 'not-found', 'Not Found', error.message);
}

export function createTechnicianHandlers(
  ...
  return {
      ...
      async (request, response, next) => {
        try {
          ...
          response.status(201).json({
            technician: await service.create(
              request.body,
              auth.profileId,
    ...
    detail: [
      ...
      async (request, response, next) => {
        try {
          ...
          if (error instanceof TechnicianServiceError) {
    ...
    update: [
      ...
      async (request, response, next) => {
        try {
          const auth = response.locals.auth as ApplicationAuth;
          response.json({
            technician: await service.update(
    ...
    availability: [
      ...
      async (request, response, next) => {
        try {
          const auth = response.locals.auth as ApplicationAuth;
          const availability = await service.replaceAvailability(
            String(request.params.id),
            request.body,
            auth.profileId,
            request.header('x-request-id') ?? undefined,
